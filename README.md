...
- [ ] **~300 more channels still show a generic/shared placeholder logo**
      (not wrong-network-mislabeled, just no distinct art in the
      provider's own catalog) — mostly the obscure `GO:`-prefixed 24/7
      channel family (~140) and the Bally/FanDuel regional variants
      beyond Wisconsin (~37). Same fix pattern as the Wisconsin Sports
      block (source a real logo, verify visually, install under both
      `poster.jpg`/`poster.png`) would work for any of these on request;
      not done for all of them given the size and how obscure/rarely-
      viewed many are.
