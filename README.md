...
- [ ] **Front-load the Packers/Badgers/Bucks/Brewers channels (owner
      request, 2026-07-21).** Identified: Bally/FanDuel Sports Wisconsin
      (ch. 600/601 — Bucks + Brewers regional home, already has real EPG),
      Big Ten Network + 3 overflow feeds (ch. 1316-1319 — Badgers),
      NFL Network/RedZone (NFL group, ch. 650+) and NBA TV (NBA group,
      ch. 700) and MLB Network (MLB group, ch. 680) for national
      coverage. Locals (Green Bay/Milwaukee/Madison, ch. 100-118) already
      carry Packers/Badgers network broadcasts and are already at the
      front. Action needed: pull the sports-network channels above out of
      their current spots into a low-number block near the locals — a
      `live_selections`/numbering change via the `renumber-xepg.py`  +
      `activate-xepg.py` cascade (not the EPG-only cascade used for alias
      work), deliberately not done same-day alongside the alias batches.
- [ ] **EPG failure notification — design proposed 2026-07-21, not yet
      built.** Today's session found two real, silently-wrong EPG bugs
      (Bally Sports West showing Miami's schedule; two German regional
      stations' data vanishing to an xid collision) that could easily
      have sat undetected for weeks, the same way the ephemeral-port bug
      sat undetected for 17h on 2026-07-16 before `media-core-healthcheck`
      existed. There's already a partial pattern to extend rather than a
      blank slate: `threadfin_ctl._write_alert()` writes
      `.threadfin_alert` on unrecovered failures, but nothing reads it —
      it's a marker for a human to stumble on, not a real notification.
      CT 107's Loki/Grafana stack (§7) already ingests CT 105's container
      logs and already has a proven push pattern (`wg-snapshot.sh` posts
      straight to Loki's HTTP push API). Proposed design: (1) push
      `_write_alert()`'s message to Loki too (`job="media-core-alerts"`),
      same pattern as `wg-snapshot.sh`, trivial addition; (2) have
      `build_epg()` log a structured per-run summary (REAL/SYNTH/PPV
      counts + each external source's matched-count, i.e. formalize this
      session's `full_gap_scan.py`/dry-run-check ad hoc scripts into a
      real part of the sync) to the same Loki stream — this also gives a
      free trending dashboard of coverage over time, not just point-in-
      time snapshots; (3) Grafana Alerting rules (Grafana's already
      running, no new services) on: any `media-core-alerts` line, any
      external source's matched-count hitting 0 (source dead/renamed/
      blocked), total REAL count dropping night-over-night by more than a
      threshold (catches silent regressions like today's collision bug
      before they sit for weeks), and the nightly sync simply not
      completing by its expected time. Open question needing the owner:
      notification channel — Grafana supports email natively (would need
      an SMTP relay decision) but no chat/push tool (ntfy, Pushover, etc.)
      is set up on this box yet, so that's a choice to make before
      building the alerting-rule side, not something to guess at.
