...
| **112** | **jellyfin-npvr** | LXC, Debian 13, unprivileged | 4 vCPU / 4 GB / 512 M swap | NextPVR stack — `jellyfin-npvr` :8096 + `nextpvr-live` :8866, plus Alloy. `onboot=1`, `/dev/dri/renderD128` passed through. Rootfs 60 G; `/srv/shared-recordings` bind-mounted. In the weekly vzdump job since 2026-08-31. |
| 110 | jellyfin-live | LXC, Debian 13, unprivileged | 4 vCPU / 4 GB | **Stopped 2026-08-02, deliberately.** Its Threadfin backend duplicated production and the fragmentation mystery it existed to control for turned out to be the pre-recording-guard bug. Retired from monitoring rather than left to alarm — see the comment block in `/root/bin/stack-monitor.py`. |
| 111 | jellyfin-vod | LXC, Debian 13, unprivileged | 2 vCPU / 8 GB | **Stopped 2026-08-31, deliberately** — see `docs/media-core-sprint-20260831.md`. Held a near-duplicate of CT 105's ~250k-item catalogue (its own 2.69 GB `jellyfin.db`) with **zero** recorded user activity, while costing ~1 core and a large share of the shared thin pool's I/O. Disk `vm-111-disk-0` (180 G) **preserved** so it is reversible; not in the vzdump job for that reason. Revival steps in the sprint doc. |
| 113 | android-emulator | LXC, Debian 13, unprivileged | — | Stopped. Used for Projectivy/launcher work via `adb`. |
...
| 2026-08-17 (scraper rewrites: all 4 deferred issues fixed) | **Closed out the entire deferred scraper backlog from earlier tonight -- Agy built a fresh, tested pass on all 4 remaining issues (11 files total); independently re-verified every claim myself before deploying, not just trusted the report.** (1) [CRITICAL] `hgtv.py`/`hallmark_drama.py` multi-day guide collapse: re-ran both live and parsed the real XML output myself -- confirmed 487/305 programmes respectively, 15.29-day span, **zero timestamp overlaps** (was 430/487 = 88% and 283/305 = 93% overlapping before). (2) `metv.py` overnight-shift + timezone leak: re-ran live, confirmed zero backward time-jumps across the full 3-day schedule (the exact symptom of the 24h-early bug) and independently recomputed the UTC-to-America/New_York conversion myself for the overnight shows -- correct, though the specific minute-level times didn't match the report's own earlier run exactly, which is expected: MeTV's live schedule page genuinely shifted slightly between the two scrapes, not a bug. (3) ESPN family (`espn`/`espn2`/`espnu`/`espnews`) JSON parsing brittleness: re-ran all 4 live, confirmed real programme data with duration capping genuinely active (`espnu`'s longest block landed exactly at the claimed 4.0h ceiling). (4) Bally off-season handling: re-ran all 4 live during genuinely-active MLB season (real Diamondbacks/Red Sox game data confirmed, not placeholder) -- the actual off-season/0-games code path couldn't be live-tested tonight since the season is active, so verified it by reading the logic directly instead (correctly gated behind `if not events`, produces one placeholder per day across the fetch window) rather than claiming a live test that wasn't possible. Deployed all 11 fixed scrapers to CT108, then ran the REAL production `run_scrapers.sh` end-to-end (not just each scraper individually) -- all 12 scrapers (including the untouched rt_news/bally_sports) reported OK. **Backout plan:** `for f in hgtv hallmark_drama metv espn espn2 espnu espnews bally_arizona bally_prime_ticket bally_great_lakes bally_sports; do sudo -n pct exec 108 -- cp /srv/scrapers/$f.py.pre-v2-rewrite-20260817 /srv/scrapers/$f.py; done` restores all 11 to their pre-rewrite state in one pass. Verify via `sudo -n pct exec 108 -- bash /srv/scrapers/run_scrapers.sh` producing all-OK again. |
| 2026-08-22 (sports-dvr-auto: cross-game trim war -- Packers @ Broncos recording destroyed by the Braves game's trimmer) | **Root-caused the 8/21 Packers @ Broncos failure (9 fragments, second half lost) to three real bugs -- full writeup in [docs/incident-20260822-packers-broncos-trim-war.md](docs/incident-20260822-packers-broncos-trim-war.md).** (1) `run_live_extender()` matched ESPN games to Jellyfin timers by TIME OVERLAP ALONE -- the Braves @ Brewers game (no timer existed; Brewers auto-record OFF) went Final and its ESPN window overlapped the Packers timer's, so every 5-min cycle trimmed the live NFL recording to now+5min; each trim killed the recording, the watchdog restored it, repeat x8; when a trim finally beat the extender the timer completed, left the watchdog's InProgress filter, and the second half never recorded. All 8 'stalls' were self-inflicted; Threadfin logged zero upstream errors (owner's 'the feed is solid' intuition was right). Fix: new `candidate_channel_ids()` -- extend/trim now requires the timer to be on a channel the game could actually air on. (2) All 8 restore confirmations failed ('created timer ID could not be confirmed') because the fallback search ran immediately after the POST, before Jellyfin listed the new timer -- so `recording-restore-state.json` never got a chain, the stitcher had nothing to stitch, and comskip processed raw fragments (comskip itself works fine). Fix: confirmation now retries 4x with 5s sleeps. Same mechanism explains the Aug 16 'MIL @ LAD 1/4 pending' stuck chain. (3) 'Packers TV Network' was unmapped: ESPN listed `['NFL Net', 'Packers TV Network', 'KUSA-TV (9NEWS)']` and the first-match loop picked NFL Network over local carriage; worse, the 8/28 Cardinals game (Packers TV Network ONLY) was being skipped as Unmapped and would have been missed. Fix: mapped it to `Green Bay: NBC 26 (WGBA)` + new `TEAM_NETWORK_BROADCASTS` priority sort in `resolve_channel_id()` so team/local carriage beats national cable regardless of ESPN's order -- verified 8/21 resolves to WGBA, 8/28 now schedules, CBS/FOX regular-season and NFL-Net-only fallbacks unchanged. Salvage: recovered a 4.96GB/2h14m `(stitched).ts` from the 7 real fragments (production concat fails on phantom audio PIDs + a torn AAC tail in the final segment -- split-stream recipe in the incident doc), archived raw segments, queued comskip. **Backout plan:** `sudo cp /root/agy-reports/sports-dvr-auto-pre-trim-war-fix-20260822.bak /usr/local/bin/sports-dvr-auto`. Verify via `python3 -m py_compile` and a dry-run pass. |
| 2026-08-22 (sports-dvr-auto: preremux phantom-PID + torn-tail hardening) | **Closed the loose end from this morning's incident: `preremux_segment_on_ct105` now survives the exact failure mode that made the 8/21 salvage manual.** (1) New `_probe_valid_stream_maps()` -- ffprobe each segment and `-map` only the first video stream + first audio stream with channels>0, skipping the provider's phantom 0-channel AAC PIDs (NFL Network HD carries two, tagged eng/spa) that a blanket `-c copy` trips over; falls back to unmapped copy if the probe fails. (2) Single-pass remux now reads with `-err_detect ignore_err -fflags +genpts+discardcorrupt`. (3) New `_preremux_split_stream_fallback()` -- on single-pass failure, extract video (mpegts) and audio (raw ADTS) separately, recombine with video copied and audio RE-ENCODED (aac 160k; mandatory -- torn AAC frames survive a bitstream copy and break the matroska mux, but the decoder skips them). Output must be genuine matroska: testing caught that an mpegts-content temp makes the downstream mkv concat SILENTLY TRUNCATE the stitch at that segment ('Can't write packet with unknown timestamp') -- an earlier draft of this fix did exactly that and was corrected before landing. Verified against the real 8/21 torn segment (fallback path, recovered full 1980.9s as true matroska), a healthy segment (fast path, no fallback triggered), and a mixed healthy+recovered 2-segment concat (correct 3608.3s, no truncation). Also confirmed this session per owner question: local broadcast-affiliate carriage (Green Bay FOX/NBC/CBS/ABC) beats national cable (ESPN/NFL Network) for Packers games in every simulcast combination tested; national cable is used only when it is the sole carriage. **Backout plan:** `sudo cp /root/agy-reports/sports-dvr-auto-pre-preremux-hardening-20260822.bak /usr/local/bin/sports-dvr-auto`. Verify via `python3 -m py_compile` and a dry-run pass. |
| 2026-08-24 (joint pipeline audit -> lock-in fixes, recordings wipe/reorg, Bayern auto-record) | **Owner asked for the Packers recording pipeline to be audited by both Claude and Agy and "locked in", the recordings library cleaned up, and a Bayern Munich auto-record toggle.** (1) AUDIT (Agy report: agy-reports/packers-pipeline-audit-20260824.md; every load-bearing claim independently reproduced before acting -- two Agy claims were wrong/imprecise and rejected: "folder.jpg is the Sports Night poster" (it was legit art; the bad poster lived in Jellyfin's metadata DB) and "timer scheduled on hdhr_106" (API timers carry GUID ChannelIds)). Deployed fixes to `/usr/local/bin/sports-dvr-auto`: [CRITICAL] trims now require positive game-identity match on timer name + slot-window guard (closes the remaining same-channel cross-game trim-war vector behind the 8/21 Packers@Broncos loss; unit-tested against the real 8/21 pairing); [CRITICAL] `find_matching_program_id` post-filters on ChannelId -- reproduced live that Jellyfin SILENTLY IGNORES `ChannelIds=hdhr_*` and returns all ~20k programs, so proximity matching could bind a Packers timer to a Hallmark movie; [HIGH] restore-stitcher no longer wedges forever on chain timers missing from timers.json (disk recovery + end+2h terminal cutoff; verified in dry-run: the 8-day-stuck 8/16 Brewers chain immediately unwedged to "Ready to stitch (4 segments)") and `failed_restore_creation` chains self-heal by timer-name adoption; [HIGH] stitched files are now enqueued for comskip (they NEVER got commercial removal before -- Jellyfin's hook only fires for its own finalized recordings) and `archive_segments` deletes each fragment's orphaned Commercial-Free cut; ESPN fetched once per cycle instead of twice; timer dedupe checks ExternalChannelId + same-name-any-channel (channel-remap dupes). (2) WIPE/REORG (owner-authorized "delete them all"): all recordings deleted (~65GB freed), `.postprocess` work/test dirs + queue cleared, segments-archive removed, restore/stall state reset to `{}`. Root cause of the "Sports Night" fiasco fixed: the mixed-content Recordings library had `EnableAutomaticSeriesGrouping=true` + default metadata fetchers, so Jellyfin TV-matched the literal folder "Sports" to the 1998 ABC show (TMDB 2003), turning game folders into "seasons" -- now `EnableAutomaticSeriesGrouping=false` + empty per-type metadata/image fetchers (verified 0 Sports Night entities after refresh). New Agy-generated neon category art installed (Sports/Series/Tivimate folder.png; kept existing Commercial Free/Other), verified via rendered contact sheet of what Jellyfin actually serves. (3) BAYERN: sports-dvr-auto gained full Bayern Munich auto-record -- TEAMS rows for ger.1/pokal/UCL, 2h30m soccer duration, ESPN /scoreboard fallback (the /schedule endpoint 400s/empties for soccer), dynamic-PPV resolution ported from dvr-dashboard Phase 3 that starts scanning provider PPV categories 6h before kickoff and schedules the labeled English slot on confident match, `candidate_channel_ids` covers DAZN PPV + Bundesliga channels; dashboard gained the "Bayern Munich" auto-record toggle (both DEFAULT_SPORTS_CONFIG copies) and the same ChannelId post-filter in its duplicate `find_matching_program_id`. Baseline German-feed fallback timer created for Friday: "Bayern Munich: VfB Stuttgart at Bayern Munich (German feed)" on Sky Sport Bundesliga 1 HD (720P), 2026-08-28 18:25Z-21:00Z, timer b15c101e confirmed. Shutdown override extended to 2026-08-29T06:00+02:00 by writing /var/lib/dvr-dashboard/override-until directly (the API caps at 24h AND a shorter POST overwrites the file -- never use the API for multi-day holds). **Backout:** `cp /usr/local/bin/sports-dvr-auto.pre-audit-fixes-20260824.bak /usr/local/bin/sports-dvr-auto`; `cp /usr/local/bin/dvr-dashboard.pre-bayern-toggle-20260824.bak /usr/local/bin/dvr-dashboard && systemctl restart dvr-dashboard`; library config: re-POST LibraryOptions for ItemId 79a2726d3c50e769a8af1e4184e4fccf with EnableAutomaticSeriesGrouping=true and empty TypeOptions; delete timer b15c101e via DELETE /LiveTv/Timers/{id}; recordings/state deletion is not reversible (owner-authorized). |
| 2026-08-24b (PPV fallback automation + pre-live-test hardening) | **Owner: "Automate the canceling of the german timer if the english slot records. It should be like that for every bayern recording."** Implemented as a two-sided mechanism in `sports-dvr-auto`: (a) `ensure_dynamic_ppv_fallback()` books a linear-channel safety net (Sky Sport Bundesliga 1 HD) for EVERY dynamic-PPV fixture at scheduling time, days ahead, independent of whether the English PPV slot has resolved -- idempotent, and it adopted the hand-made 8/28 timer rather than duplicating it because the marker string `(German feed)` was kept; (b) `check_ppv_fallback_cancellation()` cancels that fallback only once the English recording is PROVEN capturing. Deliberately conservative -- a wrong cancel loses the match, a late cancel only costs a duplicate -- so all four of: sibling timer Status=InProgress, on a curated PPV channel, overlapping window, AND >=50MB actually written to disk. Tested with an 8-case decision matrix against the real module (`scratchpad/test-fallback.py`): cancels on a healthy English recording; refuses when the English recording is InProgress-but-empty, has no file, is merely Scheduled, is on a non-PPV channel, or has a non-overlapping window; and with two fallbacks present cancels only the matching one. **Pre-live-test findings, all found by live testing against the real provider rather than by reading code:** (1) the provider returns HTTP 403 for a `Python-urllib/*` User-Agent but 200 for a custom UA on the identical URL -- the deployed code sets a custom UA so it was never broken, but `_ppv_fetch_streams` swallowed every exception and returned `[]`, making a provider/UA/network failure indistinguishable from "no slot labeled yet"; it now returns `(streams, error)` and an all-categories-failed condition logs ERROR + a `ppv_provider_unreachable` event instead of quietly downgrading to the German feed. (2) Provider slot labels carry a status prefix of exactly `Live` or `End` before the first `|` (confirmed across all six categories); the Phase 3 port ignored it entirely, so a slot still labeled `End \| Bayern Munich vs ...` from a PREVIOUS fixture could out-score and steal the booking -- finished slots are now rejected outright and `Live` scores +20. Verified live: the resolver correctly ignored three ended Bologna-Lazio slots and refused a live-but-wrong-opponent slot at 50 points (DE DAZN priority 30 + live 20), below the 130 threshold. (3) **[HIGH, unresolved by design -- owner decision]** measured curated-channel coverage: the `DAZN PPV NN` block maps 99/100 slots in category 573 (US DAZN) and **0 of 235** across categories 575/1441/1811 (UK DAZN VIP / Sky+ / UK DAZN). A confident match on a UK slot therefore scores fine and then dies at the curated-channel lookup. That is a lineup-coverage gap that will not self-heal, so it now raises ERROR + a `ppv_slot_unmapped` event + an e-mail naming the exact fix, rather than logging like an ordinary skip. Practical effect: English auto-record works today only if the match is carried on US DAZN PPV; the German fallback covers every other case. **Backout:** `cp /usr/local/bin/sports-dvr-auto.pre-ppv-fallback-20260824.bak /usr/local/bin/sports-dvr-auto` (reverts the fallback automation and all three hardening items; the 8/28 fallback timer itself survives independently as Jellyfin timer b15c101e and can be dropped with `DELETE /LiveTv/Timers/b15c101e`). |
| 2026-08-24c (faster stall detection; live-viewing recovery investigated) | **Owner asked whether a stream can be monitored so it recovers faster, after hitting playback errors in Wholphin.** Investigated jointly with Agy (report: `agy-reports/stream-monitor-readiness-20260824.md`), and the two questions turned out to have different answers. **LIVE VIEWING -- cannot be fixed server-side, confirmed from source.** Wholphin's `onPlayerError()` (verified verbatim in the real tree at `/srv/media-core/wholphin/app/src/main/java/com/github/damontecres/wholphin/ui/playback/PlaybackViewModel.kt`, ~line 1362) routes `PlayMethod.TRANSCODE` failures -- which is what Jellyfin Live TV always is -- straight into a terminal `LoadingState.Error` with no retry, which is exactly why backing out and re-clicking the channel is currently the only fix. The server cannot force recovery either: the only live Jellyfin session reports `SupportsRemoteControl: False` with no play verb in `SupportedCommands` (verified directly), and Threadfin is configured `"buffer": "-"` (verified) so it hands the client a redirect and leaves the data path entirely -- it has no byte counters or connection state to monitor. The real fix is a client-side retry in Wholphin, which the owner owns; not attempted here. (Agy's supporting *runtime log* narrative -- specific timestamps and a frozen position tick -- could NOT be verified; grep found only ExoPlayer build artifacts, no runtime log. The source-level claim above WAS verified and is what this conclusion rests on.) **RECORDINGS -- fixed.** Detection latency cut from 4-9 min to ~90s-3min: `STALL_MIN_CHECK_GAP` 4min -> 80s, plus a new dedicated `sports-dvr-stallwatch.timer` (90s, `flock`-guarded, runs only `--stall-check-only`, no ESPN calls). Safe at that interval because of real measured throughput rather than assumption: this system's own recordings run ~600-750 KB/s (Packers@Steelers 12.5GB/4h40m = 745 KB/s; stitched NFL file 617 KB/s), so 80s of healthy recording is ~48MB against an unchanged 2MB threshold -- under 5%. To keep the shorter interval from turning brief provider hiccups into needless restores (a false restore splits the recording and forces a stitch), added `STALL_STRIKES_REQUIRED = 2`: a stall must be seen on two consecutive samples before a restore, and any healthy sample resets the count. Verified with a real-module test (`scripts/test-strikes.py`): baseline sample no-ops, one stalled sample records strike 1 without restoring, a healthy sample resets to 0, and two consecutive stalls fire exactly one restore. **Correction to Agy's report:** it claimed the stall check costs "<50ms of CPU"; systemd measured `3.509s CPU` per run and an independent `/usr/bin/time` showed 86-96% CPU over ~3.8s wall -- so the true cost is ~4% of one core at a 90s cadence. Still acceptable on the N5105, but the report's figure was wrong by ~70x. **Backout:** `systemctl disable --now sports-dvr-stallwatch.timer && rm /etc/systemd/system/sports-dvr-stallwatch.{timer,service} && systemctl daemon-reload`, then `cp /usr/local/bin/sports-dvr-auto.pre-faststall-20260824.bak /usr/local/bin/sports-dvr-auto` to restore the 4-minute gap and remove the strike counter. |
| 2026-08-25 (UK PPV slots carried; probe-then-commit sequencing) | **Owner: "add the UK PPV slots to the lineup" and proposed sequencing -- once English is recordable, cancel German; start recording early during pregame to prove the channel; if that yields nothing, re-initiate German.** Also owner-confirmed the decisive constraint: **the provider entitlement is ONE concurrent stream** (independently corroborated -- Threadfin advertises `TunerCount: 1` in `/discover.json`, so Jellyfin believes it has exactly one tuner). That makes the sequencing not an optimisation but the only workable design: the English and German recordings can never coexist, so one must be given up before kickoff. **(1) UK PPV lineup.** Root cause of the measured 0/235 UK coverage was NOT a missing category -- `UK\| DAZN PPV VIP` was already in the DAZN PPV selection. The provider numbers its US and GB slots identically ("US: DAZN PPV 7" / "GB: DAZN PPV 7"), so `slot_display()` rendered both as "DAZN PPV 07" and xtream-sync's `(group, name)` dedup silently discarded every GB slot; Threadfin keys channels by name so they genuinely cannot share one. Added a `slot_name` option to `slot_display()` decoupling the string SEARCHED for in the provider name from the string DISPLAYED, then split the UK blocks into their own groups: **DAZN PPV GB** (99 channels, chno 5100) and **Sky Sports+ PPV** (70 channels, chno 5200, displayed "Sky Sports+ Event NN"). Sky needed a second pass -- the real category is `UK| SKY SPORT+ PPV ᴿᴬᵂ` with a trailing decoration char, so the `$`-anchored regex matched nothing (the sync's own "selection matched no category" warning caught it). Verified: playlist 1056 -> 1225 channels, `activate-xepg.py` activated exactly 169 new channels, Threadfin `lineup.json` now serves 1225. `sports-dvr-auto` updated to recognise the new groups (`PPV_GROUP_TITLES`, `is_ppv_channel_name()`) everywhere it previously hard-coded "DAZN PPV" -- otherwise the new slots would have been carried but still unrecordable. **(2) Probe-then-commit.** New `run_ppv_probe_commit()`: inside 40 min of kickoff, once a slot is confidently identified, it starts the English recording immediately (the probe IS the real recording, just started early), releases the single tuner by cancelling the German fallback, and then judges the result -- >=20MB written within ~4 min commits to English, otherwise it re-books German and drops English, with an 8-min-before-kickoff hard floor. Ordering is the safety story and is create-before-cancel in BOTH directions. A 7-scenario test against the real module (`scripts/test-probe.py`) covers: nothing labeled (fallback untouched), English create fails (fallback survives), clean probe start, probe proves the channel, probe yields nothing (reverts with time to spare), probe quiet but still in grace (waits rather than thrashing), and -- **a real bug the test surfaced** -- probe fails AND the German channel is missing from the lineup, where the original code cancelled English first and would have left the match with NO recording at all; it now keeps the English timer and alerts instead. **Backout:** `cp /usr/local/bin/sports-dvr-auto.pre-probe-20260825.bak /usr/local/bin/sports-dvr-auto` (removes probe/commit only); `pct exec 105 -- cp /srv/media-core/sync/config.json.pre-ukppv-20260825 /srv/media-core/sync/config.json` and the matching `xtream-sync.py.pre-ukppv-20260825` restore the pre-UK lineup, then re-run xtream-sync + activate-xepg (channels 5100-5269 disappear; nothing else moves, since every later group has its own explicit start_chno). |
| 2026-08-25b (UK PPV coverage verified end-to-end) | Closing the loop on the UK PPV lineup work above: after `xtream-sync` + `activate-xepg` the channels were live in Threadfin (`lineup.json` 1225) but **Jellyfin kept serving a cached tuner lineup of 1056** and none of the new slots were recordable. Neither Jellyfin's `Refresh Guide` task (ran to completion, Idle), nor `TasksRefreshChannels` (returned Idle in ~10s having done nothing), nor re-saving the tuner host via `POST /LiveTv/TunerHosts` (HTTP 200) dislodged it -- worth knowing, because all three look like they should. Only a `docker restart jellyfin` did; channels went 1056 -> 1225 within ~30s. Safety-checked first per the standing rule (0 sessions actively playing, 0 InProgress timers) and both Friday timers survived the restart intact. **Verified end-to-end after the restart, by cross-referencing every live provider slot to a Jellyfin channel id: 268/335 recordable, up from 99/335.** Per category: US DAZN 99/100, UK DAZN VIP 99/100 (was 0), UK Sky Sport+ 70/71 (was 0) -- the missing one in each is the `#####` header row, correctly excluded by `name_exclude`. Category 1811 (`UK\| DAZN PPV`) remains 0/64 and was deliberately NOT added: its entries are static channels ("UK: DAZN HD", "UK: DAZN 4K"), not numbered event slots, so they never carry a match-specific label for the resolver to match on and the slot mechanism does not apply to them. `sports-dvr-auto` confirms the wiring at runtime -- it now reports "Evaluating 1 fallback timer(s) against 268 curated PPV channel(s)" where it previously said 99. **Backout unchanged** from the 2026-08-25 entry above; note that reverting the lineup also requires a `docker restart jellyfin` for Jellyfin to drop the channels again, for the same caching reason. |
| 2026-08-25c (UniFi R2D2 rebuild: client cleanup, VLAN segmentation, WireGuard split-tunnel) | **Owner asked for a review of the UniFi Dream Router, which turned into a full rebuild.** Detail in `docs/unifi-rebuild-20260825.md`; highlights and the traps worth remembering: **(1) Two separate user stores.** `pve01-automation` is a UniFi OS *console admin* (mongo `ace.admin`, local-only, no cloud identity), NOT a Linux account -- grepping `/etc/passwd` for it returns nothing and proves nothing. SSH is `root`+key; the API is `pve01-automation` via `/etc/unifi-automation.auth`. Helper `/root/uni.sh` added. **(2) Topology:** the UDR is not the edge -- its WAN is `192.168.2.110` behind the GL-MT2500 which does the PPPoE. It also has only two radios (2.4/5), so any "missing third band" is a BE9300 issue. **(3) Cleanup:** 42 stale clients forgotten (128->86, snapshot at `/root/unifi-clients-snapshot-20260825.jsonl`), cross-checked against 2.1/9.1 first which saved two devices that were alive elsewhere; `USW-Flex-Mini-Finley` removed (device removal is `cmd/sitemgr` -- `cmd/devmgr` returns `rc: ok` and silently does nothing); 4 dead VPN configs deleted; and the disabled-but-still-bound Guest network deleted, which was holding `br3 = 192.168.3.1/24` **in direct conflict with the GL-BE9300's LAN IP** -- the reason `192.168.3.1` had been unreachable from pve-01. 3.1 is now reachable via new Tailscale alias `glinet-3.1-ts`. **(4) Segmentation:** VLAN 20 (IoT) and VLAN 40 (Lambeau-US) created; SSIDs `Allianz` (VLAN 1) and `Lambeau` (VLAN 40) added on all APs; `IOT` retagged to VLAN 20 keeping its original name and PSK so IoT gear reconnects with zero re-onboarding, with deliberately max-compatibility settings (WPA2 only, no WPA3/transition, PMF disabled, fast-roaming off). **(5) WireGuard on UniFi -- three real gotchas:** the private-key field is `x_wireguard_private_key` (found via `strings` on the app binary after four wrong guesses); individual fields are insufficient because DNS is then missing, so `wireguard_client_mode: "file"` with a full `.conf` is required; and UniFi rejects two clients whose interface addresses overlap, which always bites because Surfshark issues every peer the same `10.14.0.2/24` -- fixed with `/32` on both and verified empirically that Surfshark does not pin the address to the key. Credentials came from the GL routers' existing 71-peer Surfshark list, needing no account login, and both handshook first try (the assumption they were stale was wrong). **(6) [HIGH] The trap that matters most:** UniFi's traffic-route marking uses `! --match-set UBIOS_local_network dst`, which sounds protective but only covers the UDR's OWN subnets -- `192.168.9.0/24` (pve-01 and the entire media stack) is not in it, so VLAN 1 client traffic to the media stack was being marked into the Frankfurt tunnel and dropped. The owner caught this by asking why his laptop needed Tailscale to reach 9.11. **Equally important: verifying this from the router gives a false pass**, because router-originated traffic bypasses PBR entirely -- the same flaw invalidated an earlier `curl --interface` egress check. Mitigated with per-table routes into 178/179 (runtime only, may not survive re-provision); static routes via `/rest/routing` land in `main` and do NOT help, since marked traffic never consults `main`. **(7) German VPN deliberately left disabled:** routing VLAN 1 through Frankfurt trades a residential Telekom IP for a datacenter one -- same geolocation, but worse for banking/CAPTCHAs, throughput capped by the MT7622, plus the LAN fragility above. The WAN is already German, so the tunnel bought nothing; the route is built, tested and disabled, re-enabled with one toggle. `Lambeau`->Ashburn stays active (fail-closed) since a US exit is otherwise unobtainable. **Still open:** inter-VLAN isolation rules are NOT applied (VLAN 20 can still reach VLAN 1 -- deferred rather than applied untested against the media stack); `Mid-Express` remains wireless-meshed and will not serve tagged VLANs; Erin Laptop, Galaxy-A25-5G, Hisense TV and BIG-GL were still on `IOT` at retag time and should move to `Allianz`. Confirmed there is no scheduled throttling or content blocking anywhere on this router (0 firewall rules, 0 traffic rules, no bandwidth profiles), so a reported ~21:30 slowdown is not UniFi config. **Backout:** table in the doc; `IOT`'s previous network id is saved at `/root/.iot-wlan-prev-network`. |
| 2026-08-27/28 (Router cutover: BE9300 becomes PPPoE edge + LAN gateway; media stack outage and repair) | **Two router layers collapsed onto one.** The GL-BE9300 took over PPPoE from the GL-MT2500 (now retired) *and* the `192.168.9.0/24` gateway role from the GL-MT6000 (moved to `192.168.5.1`, TV corner). `192.168.2.0/24` and `192.168.3.0/24` no longer exist. Proxmox physically relocated. Full plan, scripts and rollback in `docs/cutover-20260827/`; topology map rewritten in `docs/network-topology.md`. **(1) The cutover succeeded but the media stack was dead underneath it** -- every container had no internet, with three faults stacked under tunnels that looked perfectly healthy (addresses, live handshakes, correct marks, `ip route get` returning the right tunnel): `accept_to_wgclientN` was an EMPTY fw4 chain so the forward policy `drop` killed traffic; the `oifname wgclientN jump srcnat_wgclientN` masquerade jump was missing so packets left with a private source and replies never returned (deceptive -- conntrack shows replies arriving, so it reads as routing when it is NAT); and `/tmp/resolv.conf.d/resolv.conf.wgclientN` was ZERO BYTES so the per-tunnel dnsmasq instances had no upstream and answered every lookup REFUSED while `ping` worked. All three from one root cause: **netifd never completes the "up" transition for these interfaces**, so fw4 builds the zones with no device and the DNS files are never written -- and the tunnels still work *for the router itself*, which is what made it look fine from the console. **`wg show <t> transfer` is the cheapest discriminator**: counters that do not move while a client generates traffic mean the drop is before the tunnel, i.e. firewall not VPN. **(2) Fixed self-healing, not by hand** -- `/etc/hotplug.d/iface/99-network-buildout-persist` gained `reconcile_vpn_dns()` and `reconcile_vpn_firewall()`, because `/tmp` is volatile and GL's `rtp2.sh` regenerates VPN firewall zones on every boot, wiping a one-time `uci` binding. Bind zones to the **device**, not the netifd network. **(3) Proven across four reboots**, two mine and one Agy's; the *second* is what earned its keep, exposing that the firewall binding had been wiped and only one tunnel recovered. One clean reboot is not evidence. Also rode through two real unplanned Telekom PPPoE re-auths (~01:30 and ~07:00) unattended. **(4) Earlier: an `rtp2.sh` deadlock** -- it called `/etc/init.d/network reload` from an interface-status handler, deadlocking on `procd_network.lock` with 34 stuck processes; and a stale `99-network-buildout-persist` from 2026-08-10 hardcoded `mld1 master br-iot` (putting **GIOT on the German tunnel**) and re-injected obsolete prio-5900 ip rules that forced all no-VPN LAN traffic through Zurich. **(5) Monitoring was silently blind** -- `job=router` scraped a node exporter that does not exist on either router and sat DOWN from the cutover with **no `up == 0` rule among the 8 existing alerts**, so silence read as health. Added `Infra: Prometheus target down` and `Infra: router WAN down` (5m window, deliberately longer than the nightly PPPoE re-auth). `wg-snapshot` was mislabelling all four tunnels `host=flint2`. `router-dashboard` emitted `gateway_2_1_*` for retired hardware and a false `router_subnet_conflict_detected 1`. Router syslog had been dead since 20:26 on the 27th. All repaired and verified. **pve-01 can no longer reach the UDR at `192.168.1.1`** (it is now a sibling on the far side of the UDR's WAN) -- `/root/uni.sh` falls back to Tailscale. **(6) Secrets:** `prometheus.yml` carried an inline basic-auth password -> moved to a 600 `password_file` (needed its own bind mount, since only `prometheus.yml` was mounted, as a single file); `docker-compose.yml` was **644** with credentials -> 600. The Gmail app password was never exposed (already in `.env`, 600). **(7) Two Agy findings did not survive testing** and are recorded as corrections: `wifi reload` does **not** apply a radio enable/disable on the BE9300 (measured -- VAP count unchanged, hostapd still beaconing; use `wifi down`/`wifi up`), and `wg-snapshot` was **not** failing SSH (it was mislabelling). A third, a HIGH-severity geo-DNS mismatch on the Swiss tunnel, was **disproved by measurement**: `o-o.myaddr.l.google.com TXT` from inside the containers returns the tunnel's own exit in 3/3 samples, because those Surfshark resolvers are anycast reached *through* the tunnel -- geolocating a resolver IP tells you nothing about where recursion egresses. |
| 2026-08-31 (media-core health audit + six-item sprint; CT 111 retired; hourly guide-rebuild bug fixed) | **Owner asked how healthy the media-core ecosystem was; the audit found CT 105 I/O-saturated and the sprint that followed cut host CPU idle from 29.6% to 87.3%.** Detail in `docs/media-core-sprint-20260831.md`; baseline and per-item back-out commands in `agy-reports/sprint-baseline-20260831.md`. Claude Code briefed and inspected each item, agy executed. **(1) The headline defect was not what the audit thought it was.** `dm-9` sat at ~99.8% utilisation at only ~488 KB/s -- seek-bound -- with two Jellyfin instances pegged and **no ffmpeg running**. The real cause of the recurring load was a guard in `ppv-refresh.py` that had been designed to skip Jellyfin's 16-39 min full guide rebuild when no PPV event changed, and **had never fired once in 1,137 runs**: `ET.iterparse` yields `elem.tail is None` for elements whose closing tag lands on a 16/32 KB buffer boundary, so `ET.tostring()` omits the trailing newline for those while `ppv_programmes()` always appends one; wrapped in `all()`, a handful of boundary elements out of 608 forced False every hour. Fixed by `.strip()`ing both sides (and the "updated N/M" counter, which was comparing rolling placeholder timestamps and so was never meaningful). Proven in both directions before acceptance -- negative control via Jellyfin's `ScheduledTasks` state, positive control against a mutated `<title>` in a /tmp fixture, plus idempotence. `epg.xml` and Threadfin's `update.xmltv` still run hourly **deliberately**, so `epg_age_hours` stays fresh and the staleness rule cannot false-fire; only the Jellyfin refresh is now conditional. Safe because PPV placeholders span 24 h and the internal refresh is daily. **(2) CT 111 `jellyfin-vod` retired** -- it held a near-duplicate of CT 105's ~250k-item catalogue (its own 2.69 GB `jellyfin.db`) with **zero** recorded user activity while CT 105 showed real family playback the same day. Stopped, `onboot=0`, **disk preserved** so it is reversible. Owner's call, and a better answer than the audit's original proposal to migrate VOD *onto* it. **(3) Retiring a guest leaves more behind than the monitors** -- following the CT 110 precedent covered `stack-monitor.py` and `dvr-dashboard`, but `prometheus.yml` still scraped `192.168.9.171:9100` and fired **Infra: Prometheus target down** within minutes (CT 110's `.195` was already absent -- the precedent existed and was simply not followed), and `vod-sync-ct111.timer` kept rsyncing daily into a path only CT 111 consumed. Both fixed; checklist added to `lessons-learned.md`. **(4) Also:** CT 105 Serilog `Debug`->`Information` and Media Segment Scan / Generate Trickplay Images / Extract Chapter Images disabled (log rate 1,547 -> 0 lines/60 s; the segment scan was walking all 242,474 `.strm` items to log "no providers are enabled" for every one); 7.85 GB of stale `jellyfin.db.bak-*` moved to the SATA SSD; `Environment=HOME=/root` added to `alert-responder.service`, which had been silently killing **every** automated alert investigation with `$HOME is not defined` (today's real EPG alert included) -- proven end-to-end with a synthetic webhook, then cleaned up; vzdump extended 102,105 -> 102,105,107,108,112. **(5) Five audit findings were wrong and are recorded as corrections**, because the reasoning errors generalise: `0/243` from an external EPG source means *no additional uncovered* channels, not a broken scraper (all 1225/1225 channels do carry programmes; the 424 figure is the `real=` metric, with 608 PPV and 193 synth by design); "Refresh Guide is running away" ignored `lineup-runbook.md` documenting ~20 min as normal; an "artwork-destroying `unlink`" was read off a stale 2026-08-17 proposed diff whose fix was already in both the repo and CT 105 (md5-identical); and "monitoring cannot alert" was flatly false -- Grafana alerting plus the agy webhook responder have existed since July. The genuine gap was narrower and is now closed: coverage was alerted on an **absolute** count (`real < 380`), which lineup growth silently dilutes -- 421/996 (42.3%) became 424/1225 (34.6%) while the absolute number went *up*. New `epg_coverage_ratio` / `epg_real_channels` / `epg_total_channels` gauges added to `stack-monitor.py`, sourced from the existing Loki `epg-sync` stream and degrading to *absent* rather than a false 0. **(6) Process note:** agy marked all 12 audit findings CONFIRMED including two that were demonstrably false, because it re-read the evidence it had been handed rather than testing the claims -- while separately contributing history neither the audit nor the owner had (the 2026-08-08 metadata audit proving the 139 GB artwork store is 99.999% live-referenced, so it is not reclaimable without removing libraries). Trust boundary in `agy-task.sh` and `AGENTS.md` updated to match how this actually worked. **Backout:** per-item commands in the sprint doc and baseline file; CT 111 revival is `pct set 111 --onboot 1 && pct start 111` plus restoring four `.pre-ct111-retire-20260831` backups. |
